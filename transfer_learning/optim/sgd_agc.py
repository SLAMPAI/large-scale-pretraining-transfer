...
class SGD_AGC(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False, clipping=1e-2, eps=1e-3, output_size=0):
        ...
        defaults = dict(lr=lr, momentum=momentum, dampening=dampening,
                        weight_decay=weight_decay, nesterov=nesterov, clipping=clipping, eps=eps, output_size=output_size)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                if output_size and len(p.shape) in (2,4) and p.shape[0] == output_size:
                    continue
